...
# learn to explore in DDPG
class DDPGExp(object):

    def __init__(self, actor, critic, explorer, memory, on_policy_mem,
                 observation_shape, action_shape, param_noise=None,
        ...
        self.tau = tau
        self.memory = memory
        self.on_policy_mem = on_policy_mem
        ...
        self.explorer_tf = explorer(normalized_obs0)
    ...
    # add noisy for exploration
    def pi_noisy_exp(self, obs, apply_noise=True, compute_Q=True):
        if self.param_noise is not None and apply_noise:
            actor_tf = self.perturbed_actor_tf
        else:
            actor_tf = self.actor_tf
    ...
    # explorer action, todo, add noise
    def pi_exp(self, obs, apply_noise=True, compute_Q=True):
        explorer_tf = self.explorer_tf
        feed_dict = {self.obs0: [obs]}
        if compute_Q:
            action, q = self.sess.run(
                [explorer_tf, self.critic_with_actor_tf], feed_dict=feed_dict)
        else:
            action = self.sess.run(explorer_tf, feed_dict=feed_dict)
            q = None
        action = action.flatten()
        action = np.clip(action, self.action_range[0], self.action_range[1])
        return action, q
    ...
    def store_on_policy_transition(
            self, obs0, action, reward, obs1, terminal1):
        reward *= self.reward_scale
        self.on_policy_mem.append(obs0, action, reward, obs1, terminal1)
        # todo, delete?
        if self.normalize_observations:
            self.obs_rms.update(np.array([obs0]))
    ...
    # train DDPG policy and Q use on policy data
    def train_on_policy(self):
        # Get a batch, todo set batch size larger.
        batch = self.on_policy_mem.sample(batch_size=self.batch_size)
    ...
    def clear_on_policy_mem(self):
        self.on_policy_mem.clear()
